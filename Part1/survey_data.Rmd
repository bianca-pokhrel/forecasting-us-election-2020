...
library(ggplot2)
library(tidyr) 
library(usmap) # make beautiful us state vote plot
library(knitr) # Make beautiful Latex table
library(kableExtra) # scale down tables
library(cowplot) # fit multiple graph on one page
...
vote_state <- plot_usmap(data=election_forcast, values = "vote")+
...
vote_ideo<-ggplot(cleaned_data, aes(x=ideo5, fill=trump_biden))+
  geom_bar(alpha = 0.8)+
...
vote_2016<- ggplot(cleaned_data, aes(x=vote_2016, fill=trump_biden))+
  geom_bar(alpha = 0.8)+
  facet_wrap(~trump_biden, ncol=1)+
  coord_flip()+
  theme_minimal()+
...
```{r}
plot_grid(vote, vote_state, vote_ideo, vote_2016, labels = "AUTO", ncol = 2)
```



```{r Gender, fig.cap="Vote intention by gender", fig.dim=c(5,3), echo=FALSE}
vote_gender<-ggplot(cleaned_data, aes(x=gender, fill=trump_biden))+
  geom_bar(alpha = 0.8)+
  facet_wrap(~trump_biden, ncol=1)+
  coord_flip()+
  theme_minimal()+
...
vote_race<-ggplot(cleaned_data, aes(x=race_ethnicity, fill=trump_biden))+
  geom_bar(alpha = 0.8)+
  facet_wrap(~trump_biden, ncol=1)+
  coord_flip()+
  theme_minimal()+
...
vote_age<-ggplot(cleaned_data, aes(x=as.factor(age_group), fill=trump_biden))+
  geom_bar(alpha = 0.8)+
  facet_wrap(~trump_biden, ncol=1)+
  coord_flip()+
  theme_minimal()+
...
vote_income<-ggplot(data = subset(cleaned_data, !is.na(household_income)), aes(x=factor(household_income, 
